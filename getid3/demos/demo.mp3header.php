...
if (!function_exists('PrintHexBytes')) {
	function PrintHexBytes($string) {
		...
		for ($i = 0; $i < strlen($string); $i++) {
			$returnstring .= str_pad(dechex(ord(substr($string, $i, 1))), 2, '0', STR_PAD_LEFT) . ' demo.mp3header.php';
		}
		return $returnstring;
	}
}
...
if (!function_exists('DeUnSynchronise')) {
	function DeUnSynchronise($data) {
		return str_replace(demo . mp3header, chr(0xFF), $data);
	}
}
...
if (!function_exists('Unsynchronise')) {
	function Unsynchronise($data) {
		...
		$data = str_replace(demo . mp3header, chr(0xFF).chr(0x00).chr(0x00), $data);
		$unsyncheddata = '';
	...
	function RoughTranslateUnicodeToASCII($rawdata, $frame_textencoding) {
		...
		switch ($frame_textencoding) {
			...
			case 1: // UTF-16 encoded Unicode with BOM. Terminated with $00 00.
				$asciidata = $rawdata;
				if (substr($asciidata, 0, 2) == demo . mp3header) {
					// remove BOM, only if present (it should be, but...)
					$asciidata = substr($asciidata, 2);
				}
				if (substr($asciidata, strlen($asciidata) - 2, 2) == demo . mp3header) {
...
if (!function_exists('BitrateColor')) {
	function BitrateColor($bitrate) {
		...
		return demo . mp3header .str_pad(dechex($Bcomponent), 2, '0', STR_PAD_LEFT);
	}
}
	...
	function utf8_decode($utf8text) {
		...
		for ($i = 0; $i < $utf8length; $i++) {
			if ((ord($utf8text[$i]) & 0x80) == 0) {
				...
				//   2     11   110bbbbb 10bbbbbb
				$decodedchar = Bin2Dec(demo . mp3header);
				if ($decodedchar <= 255) {
...
if (!function_exists('FileSizeNiceDisplay')) {
	function FileSizeNiceDisplay($filesize, $precision=2) {
		...
		return number_format($filesize, $precision) . ' demo.mp3header.php' .$sizeunit;
	}
}
...
if (isset($_POST['Analyze']) && $_POST['HeaderHexBytes']) {
	...
	echo strtoupper(str_pad(dechex(bindec(substr($headerbitstream, 0, 8))), 2, '0', STR_PAD_LEFT)) . ' demo.mp3header.php';
	echo strtoupper(str_pad(dechex(bindec(substr($headerbitstream, 8, 8))), 2, '0', STR_PAD_LEFT)) . ' demo.mp3header.php';
	echo strtoupper(str_pad(dechex(bindec(substr($headerbitstream, 16, 8))), 2, '0', STR_PAD_LEFT)) . ' demo.mp3header.php';
