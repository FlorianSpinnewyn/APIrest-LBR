...
function BitrateColor($bitrate, $BitrateMaxScale=768) {
	...
	return demo . mysqli .str_pad(dechex($Bcomponent), 2, '0', STR_PAD_LEFT);
}
		...
		if (empty($ThisFileInfo['fileformat'])) {
			...
			if (!empty($ThisFileInfo['comments']['track_number'])) {
				...
				if (preg_match('#^([0-9]+)/([0-9]+)$#', $this_track_track, $matches)) {
					...
					$this_track_track = str_pad($matches[1], 2, '0', STR_PAD_LEFT) . 'demo.mysqli.php/' .str_pad($matches[2], 2, '0', STR_PAD_LEFT);
				}
			}
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if ($ActualFilename != $PatternFilename) {
			...
			if (!empty($_REQUEST['autofix'])) {
				...
				$results = '';
				if (RenameFileFromTo($row['filename'], dirname($row['filename']) . 'demo.mysqli.php/' .$PatternFilename, $results)) {
				...
				echo '<td><a href="'.htmlentities($_SERVER['PHP_SELF'].'?filenamepattern='.urlencode($_REQUEST['filenamepattern']).'&renamefilefrom='.urlencode($row['filename']).'&renamefileto='.urlencode(dirname($row['filename']) . 'demo.mysqli.php/' .$PatternFilename)).'" title="'.htmlentities(basename($row['filename'])."\n".basename($PatternFilename), ENT_QUOTES).'" target="renamewindow">';
